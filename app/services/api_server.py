"""
API Сервер (Quart/Flask).
Це застаріла частина перейшла з початкового проекту
Забезпечує веб-інтерфейс або API для віддаленого моніторингу та керування системою.
"""
...
class ApiServer:
    ...
    def __init__(self, settings: ApiServerSettings):
        ...
        self.on_rf_data = lambda data: print(
            "Попередження: обробник для RF даних не встановлено."
        )
        self.on_audio_alert = lambda status: print(
            "Попередження: обробник для аудіо-тривоги не встановлено."
        )
        ...
        self.quart_app.route("/api/audio_alarm", methods=["POST"])(
            self.receive_audio_alarm
        )
