...
def test_update_data_rf(dynamic_chart):
    # Частотна вісь: [CenterFreq - SampleRate/2, CenterFreq + SampleRate/2]
    ...
    assert len(x) == 512
    assert x[0] == 910.0  # (915e6 - 10e6 / 2) / 1e6
    assert x[-1] == 920.0  # (915e6 + 10e6 / 2) / 1e6

    assert np.array_equal(dynamic_chart.waterfall_buffer[0], data)


def test_handle_resize(dynamic_chart):
    # Зміна роздільної здатності FFT
